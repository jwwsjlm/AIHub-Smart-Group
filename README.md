...
在 API 密钥列表中打开网站原生的“选择分组”下拉框时，每个分组会在同一行显示名称、供应商大厅最新状态、倍率和最新首 Token。首 Token 数值使用绿色加粗显示，长分组名会自动省略，倍率和速率保持可见。可用状态为绿色，有监控警告为橙色，不可用为红色，已停用或暂无监控为灰色。没有延迟数据时显示“首 Token 暂无数据”。这些信息只用于提示，不会禁用分组、改变排序或影响原有搜索和选择操作；弹层打开期间最多每 60 秒刷新一次。
...
桌面端面板使用左右双栏：左侧用于检测、推荐和密钥切换，右侧通过“设置 / 日志”标签切换内容。两栏分别滚动，面板不会因为打开设置或查看大量日志而超出屏幕顶部。脚本会记住最后打开的“设置”或“日志”标签。窄屏设备会自动恢复为单列，并在面板内部滚动。
