...
平衡模式不是简单取“价格和速度的平均值”，而是分两步选择：

1. 先按当前可靠性设置过滤分组，例如当前状态、最近 10 分钟可用率和监控警告。
2. 在剩余分组中找到最低倍率，并按“最低倍率 ×（1 + 平衡价格范围）”计算价格上限。
3. 只在不超过价格上限的分组中，选择首 Token 最快的一个。

默认“平衡价格范围”为 `20%`。例如当前最低可用倍率为 `0.04`：

- 设置 `0%`：价格上限是 `0.04`，只在最低价分组中比较速度。
- 设置 `20%`：价格上限是 `0.048`，会在倍率不超过 `0.048` 的分组中选择最快的。
- 设置 `50%`：价格上限是 `0.06`，会在倍率不超过 `0.06` 的分组中选择最快的。
- 设置 `100%`：价格上限是 `0.08`，速度选择范围更大，但可能明显增加费用。
...
2. 将“模式”选择为“平衡”。
3. 展开“设置”。
4. 在“平衡价格范围（%）”中填写数字，例如 `20` 表示允许比最低倍率高 `20%`。
5. 点击“保存设置”，等待重新检测。

输入百分比时，设置下方会实时显示当前最低可用倍率和预计价格上限，例如：`当前输入 20% · 最低 ×0.04 · 预计上限 ×0.048`。输入阶段仅用于预览，出现“未保存”时不会改变推荐或自动切换目标；点击“保存设置”后才会正式生效。

保存后，平衡模式推荐卡片会显示选择依据，例如：`范围内首 Token 最快 · 最低 ×0.04 · +20% · 上限 ×0.048`。这里的“上限”是计算出的理论上限，不代表一定存在倍率恰好等于该数值的分组。

建议先使用默认的 `20%`。更重视价格时可设置为 `0–10%`；希望用少量价格换取更快首 Token 时可设置为 `20–50%`。设置越高，候选范围越大，最终倍率也可能越高。

平衡价格范围只控制价格上限，不会绕过可用性和警告过滤。所选模式会同时影响推荐卡片、手动切换和自动切换目标。
