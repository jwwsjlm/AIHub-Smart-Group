...
桌面端面板使用左右双栏：左侧用于检测、推荐和密钥切换，右侧通过“设置 / 日志”标签切换内容。两栏分别滚动，面板不会因为打开设置或查看大量日志而超出屏幕顶部。窄屏设备会自动恢复为单列，并在面板内部滚动。
...
“排除分组关键词”使用 `|` 分隔多个关键词，例如 `free|unstable`。匹配忽略大小写，只要分组名称包含其中任一关键词，该分组就不会参与价格、平衡或速度模式的推荐和切换。切换冷却支持小数，例如 `0.1` 分钟表示 6 秒。
