...
可用性判断默认使用最近 10 分钟可用率。由于 AIHub 监控通常每 5 分钟产生一个点，也可以切换为“成功监控点数”或“连续成功点数”：前者表示窗口内累计成功多少个点，后者表示最近的连续成功点数。例如设置为 `2`，前者允许前后两个点成功，后者要求最近两个点都连续成功。下拉框右侧会紧凑显示当前规则摘要，并随选择和输入实时更新。
