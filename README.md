...
两个面板都提供“快速开始”和持久化使用日志。日志记录检测、模式、设置、推荐、切换结果和错误摘要，最多保留最近 100 条，不保存完整 API Key。
...
帅API 支持三种目标模式：价格选择最低倍率；平衡在最低倍率的可调价格范围内选择首字最快；速度直接选择首字最快。模式、平衡价格范围、自动选择目标、时间窗、成功率、稳定性、最近数据和刷新间隔都会保存。
...
- AIHub 支持价格、平衡和速度模式；模式会同时影响推荐、手动切换和自动切换目标。
- 平衡模式在最低倍率的可调价格范围内选择首 Token 最快的分组。
- 当前监测状态必须为可用。
- 6 小时可用率至少 `95%`。
- 24 小时可用率至少 `90%`。
- 默认排除带监控警告的分组。
- 价格模式按倍率升序排列；同倍率时依次比较 6 小时可用率、24 小时可用率、首 Token 延迟。
