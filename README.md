...
在 API 密钥列表中打开网站原生的“选择分组”下拉框时，每个分组名称下方会显示供应商大厅的最新状态，倍率下方会显示最新首 Token。可用状态为绿色，有监控警告为橙色，不可用为红色，已停用或暂无监控为灰色。没有延迟数据时显示“首 Token 暂无数据”。这些信息只用于提示，不会禁用分组、改变排序或影响原有搜索和选择操作；弹层打开期间最多每 60 秒刷新一次。
